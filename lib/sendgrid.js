 ...
 * @param {Object}  options either https agent options or http proxy options: { maxSockets: 500 , proxy: { host: 'localhost', port:3128 }}
 */
function SendGrid(api_user, api_key, options) {
  this.api_user = api_user;
  this.api_key = api_key;
  this.options = options || { maxSockets: 20 };
  this.proxy = this.options.proxy || null;
  this.setupAgent();
}

//Sets up either proxy agent or http agent
SendGrid.prototype.setupAgent = function() {
  var self = this;
  if(self.proxy) {
    self.agent = tunnel.httpsOverHttp({
      maxSockets: self.options.maxSockets,
      proxy: { 
        host: self.proxy.host || 'localhost',
        port: self.proxy.port || 3128
      }
    });
    if(self.proxy.auth)
      self.agent.proxy.proxyAuth = self.proxy.auth; //enable auth if auth string exists
    return
  }
  self.agent = new https.Agent({maxSockets: self.options.maxSockets }); //In future allow overrides
}
