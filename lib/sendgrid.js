  ...
  var _send = function(email, callback) {
    ...
    for (var field in form) {
      ...
      if (value && value.filename) {
        ...
        if (reqForm.hasOwnProperty('files[' + value.filename + ']')) {
          var oldFileName = value.filename.split('.');
          var newFileName = oldFileName[0] + '_' + count + '.' + oldFileName[1];
          reqForm.append("files["+newFileName+"]", value.content, {filename: newFileName, contentType: value.contentType});
          count++;
        } else {
