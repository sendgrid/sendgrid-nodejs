...
function SendGrid(api_user, api_key, options) {
  ...
  this.maxSockets = options && options.maxSockets || 20;
  this.proxy = options && options.proxy || null;
  this.setupAgent();
}
...
SendGrid.prototype.setupAgent = function() {
  if(this.proxy) {
    this.agent = tunnel.httpsOverHttp({
      ...
      proxy: { 
        host: this.proxy.host || 'localhost',
        port: this.proxy.port || 3128
      }
    });
    if(this.proxy.auth)
      this.agent.proxy.proxyAuth = this.proxy.auth; //enable auth if auth string exists
    return
  }
  this.agent = new https.Agent({maxSockets: this.maxSockets }); //In future allow overrides
}
