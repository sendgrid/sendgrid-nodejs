...
var Sendgrid = function(api_user, api_key, options) {
  ...
  var _sendWeb = function(email, callback) {
    ...
    // Merge global request options and endpoint specific options
    // Leaving merge sync since it is simple
    var options = _.merge(postOptions, this.options.web);
    ...
    // Moved outside of block and loop
    var _reqFormAppend = function(field, each) {
  ...
  var _sendSmtp = function(email, nodeMailerOptions, callback) {
    ...
    smtpTransport.sendMail(smtpParams, function(error) {
      //error, response returns but response not currently used
      smtpTransport.close();
