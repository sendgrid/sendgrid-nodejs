...
SendGrid.prototype.setupAgent = function() {
  if(this.options.proxy) {
    this.agent = tunnel.httpsOverHttp({
      ...
      proxy: { 
        host: this.options.proxy.host || 'localhost',
        port: this.options.proxy.port || 3128
      }
    });
    if(this.options.proxy.auth)
      this.agent.proxy.proxyAuth = this.options.proxy.auth; //enable auth if auth string exists
    return
  }
  this.agent = new https.Agent({maxSockets: this.options.maxSockets }); //In future allow overrides
}
