...
Email.prototype.processFiles = function(callback) {
  var self = this;
  // number of attachments to loop through
  var attachments_count = _.size(this.files);

  _(this.files).each(function(file) {
    // for each file lets check to see what type it is and handle it there
    if (file.type === 'path') {
      // start of path file type
      ...
      // end of path file type
    } else if (file.type === 'url') {
      // start of url file type
      ...
      var req = http.request({host: host, path: path, method: 'GET'}, function(res) {
        // decrement attachments_count, maybe shouldn't do this here?
        attachments_count--;
...
Email.prototype.toSmtpFormat = function() {
  ...
  console.log('returning data in toSmtpFormat');
  return data;
};
