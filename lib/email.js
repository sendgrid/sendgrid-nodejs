...
Email.prototype.toWebFormat = function() {
  ...
  if (this.files) {
    ...
    this.files.forEach(function(file) {
      ...
      // if there are multiple files of same name add a counter to name part
      if (web.hasOwnProperty('files[' + file.filename + ']')) {
        var oldFileName = file.filename.split('.');
        var newFileName = oldFileName[0] + '_' + count + '.' + oldFileName[1];
        web['files[' + newFileName +']'] = {
