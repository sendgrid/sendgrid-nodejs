...
Email.prototype.toSmtpFormat = function() {
  ...
  if(this.fromname != null){
    data.sender = formatNameAndEmail(this.fromname, this.from);
  }
  if(_(this.toname).isArray() && _(this.to).isArray()){
    if(this.toname.length != this.to.length){
      throw new Error("To and Toname arrays must be equal length")
    }
    var _this = this;
    data.to = _(this.to).map(function(to, index){
      return formatNameAndEmail(_this.toname[index], to)
    });
  } else if(this.toname != null){
    data.to = formatNameAndEmail(this.toname, this.to);
  } 
...
var formatNameAndEmail = function(name,email) {
  return name + " <" + email + ">";
};
