 ...
 * Dependencies
 */
const {MailService} = require('@sendgrid/mail'); //This may not work as expected since it was previously sendgrid
 ...
 class SendGridTransport {
  ...
  constructor(options = {}) {
    ...
    if (!this.options.auth.api_user) {
      // api key
      this.sendgrid = MailService();
      this.sendgrid.setApiKey(this.options.auth.api_key);
    } else {
      // username + password
      this.sendgrid = MailService();
      this.sendgrid.setClient(this.options.auth.api_user); //May not work as expected
      this.sendgrid.setApiKey(this.options.auth.api_key);
    }
  }
